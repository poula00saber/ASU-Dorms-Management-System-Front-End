...
export default function StudentsList() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const json = await fetchAPI("/api/Students");
  ...
  // Delete student using fetchAPI - FIXED VERSION
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      // Use fetchAPI instead of direct fetch
      const response = await fetchAPI(`/api/Students/${id}`, {
        method: "DELETE",
      });
  ...
  // Alternative delete function if the above doesn't work
  const handleDeleteAlternative = async (id: string, name: string) => {
    if (!window.confirm(`هل أنت متأكد من حذف الطالب ${name}؟`)) return;
    ...
    try {
      ...
      const selectedDormId =
        localStorage.getItem("selectedDormId") || userInfo?.dormLocationId;
                  ...
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        >
                          {deletingId === s.id ? (
                            <span className="animate-spin inline-block w-5 h-5 border-2 border-red-600 border-t-transparent rounded-full"></span>
                          ) : (
